...
- **📅 智能查询** — 自动算今天是第几周,回答"今天/明天/某周周X 有什么课",带学分统计
  ...
  单日每个大节输出三行(课程名称/课程教室/任课老师),从上到下依次排列。
...
- **🗓️ 整周网格视图** — `--week 3` 七列(周一~日)×五行(五大节)网格,课程/教室/教师一目了然
  ...
  单周网格为七列(周一~周日)×五行(五大节),每格三行(课程名称/课程教室/任课老师),**教室加粗**。
...
- **📐 输出格式规范化(v1.2.2)** — 单日每大节三行(课程名称/课程教室/任课老师),单周七列×五行网格中教室加粗
...
| 方式 | 说明 |
|------|------|
| **zip 包(推荐)** | [cqut-timetable-skill-1.2.3.zip](dist/cqut-timetable-skill-1.2.3.zip) — 解压即用,含全部 11 个文件 |
...
之后直接对话(Agent 运行 `today_classes.py`,**原样展示脚本输出**):
...
> 问整周课表时,Agent 同样**原样展示**七列×五行网格(教室已用 `**` 加粗),示例见上方「整周网格视图」。
