...
- **📅 智能查询** — 自动算今天是第几周,回答"今天/明天/某周周X 有什么课",带学分统计
  ...
  | 节次 | 课程 | 教室 |
  |---|---|---|
  | 1-2节 08:20-10:00 | 示例课程A | **1教0101** |
  | 5-6节 14:00-15:40 | 示例课程C | **3教0303** |

  🎓 今日学分 6.5 / 学期总学分 18.75 (34.7%)
  ```

  单日为三列 markdown 表格(节次+时间段 / 课程 / **教室**),教室粗体,紧凑。
...
- **🗓️ 整周网格视图** — `--week 3` 七列(周一~日)表格,每大节一行,格内 `课程 / **教室**`,节次列带时间段
  ...
  |---|---|---|---|---|---|---|---|
  | 1-2节 08:20-10:00 |  | 示例课程A / **1教0101** | 示例课程B / **2教0202** |  | 示例课程D / **操场** |  |  |
  | 3-4节 10:20-12:00 | 示例课程E / **1教0102** |  |  |  |  |  |  |
  | 5-6节 14:00-15:40 |  |  |  |  |  |  |  |
  | 7-8节 16:00-17:40 |  |  |  |  |  |  |  |
  | 9-10节 |  |  |  |  |  |  |  |
  ...
  单周为七列 markdown 表格(周一~周日),每大节一行,格内 `课程 / **教室**`,节次列带时间段,**教室加粗**。
...
- **👥 多教师省略号(v1.2.0)** — `--list` 中多位教师(如 `张老师,李老师`)只显示第一个+`…`(日常单日/单周表格不含教师)
...
> 问整周课表时,Agent 同样**原样展示** `--markdown` 输出的七列 markdown 表格(每大节一行,教室已用 `**` 加粗,渲染后为粗体)。
...
├── today_classes.py             # 查询工具: 大节映射 + 单双周解析 + 学分统计 + 单日三列/单周七列表格输出(零依赖)
