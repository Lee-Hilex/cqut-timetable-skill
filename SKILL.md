...
- **今天**: 不带参数 → 单日 markdown 块(每大节: 加粗标题+三行)
- **指定日期**: `--date 2026-09-23` → 单日 markdown 块
- **指定周次+星期**: `--week 15 --day 4`(day: 1=周一 ... 7=周日) → 单日 markdown 块
- **整周课表**: `--week 3`(不带 `--day`) → 七列×五行 markdown 表格
- **全部课程**: `--list` → 横向表格(旧格式,含周次/学分完整信息)
- **指定课表文件**: `--schedule schedule_2025_2.json`(春季学期时)

> ⚠️ **输出即原样**: 脚本输出已是规范的 markdown 表格,每个大节占三行(课程名称/教室/教师),教室已用 `**` 加粗。**直接把脚本打印的内容原样贴给用户**(不需要代码块包裹),markdown 会渲染成对齐的表格,`**教室**` 渲染成粗体。
...
- **单日** (`--week N --day D` 或默认今天): 每个大节一个 markdown 块:加粗标题行(大节标签+时间)+ 三行(课程名称/课程教室/任课老师,从上到下)
- **整周** (`--week N`): 七列(周一~周日)markdown 表格,**每个大节占三行**(课程名称行/课程教室行/任课老师行),教室已被脚本用 `**教室**` 加粗——**原样保留 `**`**,渲染后显示粗体。**不要用 `<br>`**(raw HTML 会被聊天界面转义成字面量,已多次踩坑)
