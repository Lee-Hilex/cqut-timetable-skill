...
> ⚠️ **输出即原样**: 脚本输出已是规范的 markdown 表格,教室已用 `**` 加粗。**直接把脚本打印的内容原样贴给用户**(不需要代码块包裹),markdown 会渲染成对齐的表格,`**教室**` 渲染成粗体。
...
- **整周** (`--week N`): 七列(周一~周日)×五行(五大节)markdown 表格,每格三行(课程名称/课程教室/任课老师,用 `<br>` 分隔),教室已被脚本用 `**教室**` 加粗——**原样保留 `**`**,渲染后显示粗体
